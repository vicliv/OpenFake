...
def main():
    ...
    all_masks = glob.glob(os.path.join(MASKS_DIR, "*.png"))
    print(f"Found {len(all_masks)} masks to process.", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(expand_single_mask, p): p for p in all_masks}
    ...
    print(f"\nDone. {success} expanded, {failed} failed out of {len(all_masks)} total.", flush=True)

    # Write marker file
    with open(MARKER_FILE, "w") as f:
        f.write(f"Expanded {success} masks on {__import__('datetime').datetime.now().isoformat()}\n")
    print(f"Wrote marker file to {MARKER_FILE}", flush=True)
